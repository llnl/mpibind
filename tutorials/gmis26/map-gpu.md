# Hardware Affinity for Applications:<br> Adding in GPU kernels
...
An hwloc-rendered image of the Frontier architecture is [here](../figures/frontier-web.png).

## Affinity via the resource manager

In the [previous module](map-cpu.md) we covered CPU affinity using Slurm. In this section, we extend our coverage of Slurm-enabled CPU affinity to include GPU affinity.
...
launching commands from a login node rather than an allocated node.
...
Now, let's try it on a different [machine](../figures/aws-g4dn-metal.png).
...
Now, let's try Frontier. Run either:
* Two tasks per GPU
* Two GPUs per task.
...
Answer: Two tasks per GPU
</summary>

```
$ srun --gpus-per-node=8 -t1 -N1 -n16 -c3 --cpu-bind=thread --gpu-bind=single:2 mpi+gpu
  0 frontier07429   3 CPUs: 1-3
  0 frontier07429   1 GPUs: 0000:d1:00.0
  1 frontier07429   3 CPUs: 9-11
  1 frontier07429   1 GPUs: 0000:d6:00.0
  2 frontier07429   3 CPUs: 17-19
  2 frontier07429   1 GPUs: 0000:c9:00.0
  3 frontier07429   3 CPUs: 25-27
  3 frontier07429   1 GPUs: 0000:ce:00.0
  4 frontier07429   3 CPUs: 33-35
  4 frontier07429   1 GPUs: 0000:d9:00.0
  5 frontier07429   3 CPUs: 41-43
  5 frontier07429   1 GPUs: 0000:de:00.0
  6 frontier07429   3 CPUs: 49-51
  6 frontier07429   1 GPUs: 0000:c1:00.0
  7 frontier07429   3 CPUs: 57-59
  7 frontier07429   1 GPUs: 0000:c6:00.0

  8 frontier07429   3 CPUs: 4-6
  8 frontier07429   1 GPUs: 0000:d1:00.0
  9 frontier07429   3 CPUs: 12-14
  9 frontier07429   1 GPUs: 0000:d6:00.0
 10 frontier07429   3 CPUs: 20-22
 10 frontier07429   1 GPUs: 0000:c9:00.0
 11 frontier07429   3 CPUs: 28-30
 11 frontier07429   1 GPUs: 0000:ce:00.0
 12 frontier07429   3 CPUs: 36-38
 12 frontier07429   1 GPUs: 0000:d9:00.0
 13 frontier07429   3 CPUs: 44-46
 13 frontier07429   1 GPUs: 0000:de:00.0
 14 frontier07429   3 CPUs: 52-54
 14 frontier07429   1 GPUs: 0000:c1:00.0
 15 frontier07429   3 CPUs: 60-62
 15 frontier07429   1 GPUs: 0000:c6:00.0
```
</details>
...
Answer: Two GPUs per task
</summary>

```
$ srun --gpus-per-node=8 -t1 -N1 -n4 -c14 --cpu-bind=thread --gpu-bind=per_task:2 mpi+gpu
  0 frontier07441  14 CPUs: 1-7,9-15
  0 frontier07441   2 GPUs: 0000:d1:00.0 0000:d6:00.0
  1 frontier07441  14 CPUs: 17-23,25-31
  1 frontier07441   2 GPUs: 0000:c9:00.0 0000:ce:00.0
  2 frontier07441  14 CPUs: 33-39,41-47
  2 frontier07441   2 GPUs: 0000:d9:00.0 0000:de:00.0
  3 frontier07441  14 CPUs: 49-55,57-63
  3 frontier07441   2 GPUs: 0000:c1:00.0 0000:c6:00.0
```
</details>
...
```
$ srun --gpus-per-node=8 -t1 -N1 -n8 -c7 --cpu-bind=thread --gpu-bind=closest mpi+gpu
```
</summary>

```
  0 frontier04673   1 GPUs: 0000:d1:00.0
  0 frontier04673   7 CPUs: 1-7
  1 frontier04673   1 GPUs: 0000:d6:00.0
  1 frontier04673   7 CPUs: 9-15
  2 frontier04673   1 GPUs: 0000:c9:00.0
  2 frontier04673   7 CPUs: 17-23
  3 frontier04673   1 GPUs: 0000:ce:00.0
  3 frontier04673   7 CPUs: 25-31
  4 frontier04673   1 GPUs: 0000:d9:00.0
  4 frontier04673   7 CPUs: 33-39
  5 frontier04673   1 GPUs: 0000:de:00.0
  5 frontier04673   7 CPUs: 41-47
  6 frontier04673   1 GPUs: 0000:c1:00.0
  6 frontier04673   7 CPUs: 49-55
  7 frontier04673   1 GPUs: 0000:c6:00.0
  7 frontier04673   7 CPUs: 57-63
```
</details>
-->
...
Let's use Frontier to try out this option.

First, calculate a couple of masks and then, use them to assign a task to
...
```
$ srun --gpus-per-node=8 -t1 -N1 -n2 -c7 --cpu-bind=thread --gpu-bind=mask_gpu:0x03,0x30 mpi+gpu
```
</summary>

```
  0 frontier08131   7 CPUs: 1-7
  0 frontier08131   2 GPUs: 0000:c1:00.0 0000:c6:00.0
  1 frontier08131   7 CPUs: 9-15
  1 frontier08131   2 GPUs: 0000:d1:00.0 0000:d6:00.0
```
</details>
...
* We want our parallel program to leverage at least one GPU on each
  NUMA domain. So, we can hand-pick a GPU from each domain and map the
  parallel program to those. Below, GPU 0 (from NUMA 3) and GPU 4 (from NUMA 0) have been specified with `--gpu-bind=map_gpu:0,4`. At least two tasks (`-n`) are needed.

```
$ srun --gpus-per-node=8 -t1 -N1 -n2 -c7 --cpu-bind=thread --gpu-bind=map_gpu:0,4 mpi+gpu
  0 frontier06620   7 CPUs: 1-7
  0 frontier06620   1 GPUs: 0000:c1:00.0
  1 frontier06620   7 CPUs: 9-15
  1 frontier06620   1 GPUs: 0000:d1:00.0
```
...
GPU vendors provide environment variables to restrict the set of GPUs a process or thread can use. The environment variables AMD and NVIDIA provide for their devices are the following:
...
Let's try this on Frontier.

<details>
<summary>

```
$ srun -t1 -N1 -n4 -c7 --cpu-bind=thread --gpu-bind=per_task:1 sh -c 'ROCR_VISIBLE_DEVICES=7,6,5,4 mpi+gpu'
```
</summary>

```
  0 frontier09431   7 CPUs: 1-7
  0 frontier09431   4 GPUs: 0000:de:00.0 0000:d9:00.0 0000:d6:00.0 0000:d1:00.0
  1 frontier09431   7 CPUs: 9-15
  1 frontier09431   4 GPUs: 0000:de:00.0 0000:d9:00.0 0000:d6:00.0 0000:d1:00.0
  2 frontier09431   7 CPUs: 17-23
  2 frontier09431   4 GPUs: 0000:de:00.0 0000:d9:00.0 0000:d6:00.0 0000:d1:00.0
  3 frontier09431   7 CPUs: 25-31
  3 frontier09431   4 GPUs: 0000:de:00.0 0000:d9:00.0 0000:d6:00.0 0000:d1:00.0
```
</details>
...
<!--
because sometimes the variable has
...
```
$ srun --gpus-per-node=4 -t1 -N1 -n4 -c7 --cpu-bind=thread --gpu-bind=per_task:1 sh -c 'ROCR_VISIBLE_DEVICES=7,6,5,4 mpi+gpu'
```
</summary>
...
#### Hands-on exercise B: Crossing NUMA domains with env variables

As in exercise A, run a parallel program that uses a GPU from different NUMA domains on a single node of Frontier. This time, use an environment variable to select your GPUs instead of `map_gpu`.
...
Here, we use `ROCR_VISIBLE_DEVICES` to specify two GPUs -- one from each NUMA domain. This might look like

```
$ srun -t1 -N1 -n2 -c7 --cpu-bind=thread --gpu-bind=per_task:1 sh -c 'ROCR_VISIBLE_DEVICES=0,4 mpi+gpu'
  0 frontier08140   7 CPUs: 1-7
  0 frontier08140   2 GPUs: 0000:c1:00.0 0000:d1:00.0
  1 frontier08140   7 CPUs: 9-15
  1 frontier08140   2 GPUs: 0000:c1:00.0 0000:d1:00.0
```

Here `gpu-bind=per_task:1` isn't working, as expected. We are using two GPUs in our parallel program -- one from each NUMA domain -- but each task has access to both GPUs. In exercise A, in contrast, each task had its own GPU.
...
To be complete, can you think of how to assign local CPUs as well? For
example, task 1 is running on `CPUs 8-15`, which are in `NUMA 0`,
but `GPU 0xce` is in `NUMA 1`.

#### Hands-on exercise C: local CPUs & GPUs together

Using environment variables, run 4 tasks such that each task run on a unique NUMA domain and a local GPU.

Hint:
* Leverage the script from Example 6.
* Then run `srun <options> ./assign_gpus mpi+gpu`
...
```
$ srun -t1 -N1 -n4 --cpu-bind=map_cpu:9,25,41,57 ./assign-gpus mpi+gpu
  0 frontier09443   1 CPUs: 9
  0 frontier09443   1 GPUs: 0000:d6:00.0
  1 frontier09443   1 CPUs: 25
  1 frontier09443   1 GPUs: 0000:ce:00.0
  2 frontier09443   1 CPUs: 41
  2 frontier09443   1 GPUs: 0000:de:00.0
  3 frontier09443   1 CPUs: 57
  3 frontier09443   1 GPUs: 0000:c6:00.0
```

</details>


Bonus points: Use *all* the cores local to the chosen GPUs.
...
```
$ srun -t1 -N1 -n2 -c7 --cpu-bind=thread ./assign-gpus mpi+gpu
  0 frontier09443   7 CPUs: 1-7
  0 frontier09443   1 GPUs: 0000:c1:00.0
  1 frontier09443   7 CPUs: 9-15
  1 frontier09443   1 GPUs: 0000:d1:00.0
```
...
1. Using Slurm's affinity, use two cores per MPI task and assign a local GPU to each task. A GPU may be assigned to more than one task.

   Here is some guidance:

   * Frontier has 56 cores, so you'll want to specify 24 tasks.
   * Each NUMA domain may host 6 tasks, 3 per L3 cache.
   * You can use this [image](../figures/frontier-web.png) to understand which GPUs & CPUs are on each NUMA domain.
...
2. Using Slurm's GPU affinity or vendor-specific environment
   variables, run a 3-task MPI program such that 2 tasks reside on one
   NUMA domain and use local GPUs (one GPU per task) and 1 task reside
   on a different NUMA domain and use local GPUs (two GPUs per
   task). If using environment variables to control GPU affinity, you
   can still use Slurm to control CPU affinity.
...
[amd]: https://rocm.docs.amd.com/en/docs-7.2.4/conceptual/gpu-isolation.html
